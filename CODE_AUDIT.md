...
### 5. `fromEvent()` puts primary Redis client into subscriber mode
...
`client.subscribe()` is called on `this.clients[0]` — the same client used for ALL lock operations (`acquire`, `release`, `extend`, `get`, `publish`, `getStatus`). In ioredis, once a client enters subscriber mode it **cannot execute regular commands**. This will break every lock operation that uses the first client, effectively making the entire toolkit non-functional after a single `fromEvent()` call.

### 6. `waitForMessage` has broken `const` re-assignment — cleanup never works
**File:** `src/pubsub/pubsub-manager.ts:151,181`
**Severity:** CRITICAL

```ts
const onMessage: ((message: string) => void) | undefined;  // line 151 — declared as const
// ...
onMessage = (message: string) => { ... };  // line 181 — reassignment of const!
```

`onMessage` is declared as `const` on line 151 but reassigned on line 181. If TypeScript compiles this (e.g., with `let` behavior at runtime), the `localCleanup` closure on line 155 captures the initial `undefined` value of `onMessage`, so `if (onMessage)` is always false. The listener is never removed from the `channelListeners` Set, causing a **memory leak** — every `waitForMessage` call accumulates an orphaned listener.

---

## High Severity Issues

### 7. `forceRelease` has a race condition with `totalReleased` counter
...
### 8. `getClientForRead` always returns first client — no load balancing
...
### 9. `ioredis` in both `dependencies` and `peerDependencies`
...
### 10. Optimistic locking operations are NOT consensus-based
...
### 11. CacheManager `getStats()` returns hardcoded zeros
...
### 12. CacheManager `set()` is never called from the main flow
...
### 13. Lock `onError` callback in `using()` doesn't propagate the error
...
### 14. `InternalRedlockLock.using()` calls `release()` twice
...
### 15. `validateConfig` uses falsy check for `retryCount`
...
### 16. PubSub waiter only listens on `resources[0]`
...
### 17. `cleanup()` may delete non-lock keys
...
### 18. `OPTIMISTIC_WRITE_SCRIPT` — empty string TTL issue
...
### 19. `preloadScripts` returns a promise that's never awaited
...
### 20. `Date.now` monkey-patch without cleanup guard
...
### 21. `fromEvent` subscribes but never tracks subscriptions
**File:** `src/index.ts:875-904`
**Severity:** LOW
...
### 22. Always-passing assertion in stress test
...
### 23. Misleading test name in negative cases
...
### 26. No test coverage for `acquireHybrid()`, cache methods, or compatibility API
**Severity:** HIGH
...
### 27. Duplicate keyword in package.json
**File:** `package.json:37,44`
**Severity:** LOW

`"fault-tolerance"` appears twice in the `keywords` array.

### 28. `tsconfig.build.json` has invalid negation pattern
...
### 29. Examples import from `../src/index` instead of package name
**File:** `examples/*.ts`
**Severity:** MEDIUM
...
### 30. Examples use single Redis instance with different `db` values
**File:** `examples/*.ts`
**Severity:** MEDIUM
...
|----------|----------|------|--------|-----|
| Security/Race Conditions | 3 | 1 | 2 | 0 |
| Logic Bugs | 2 | 2 | 4 | 0 |
...
1. **[CRITICAL]** Fix `fromEvent()` — must use dedicated subscriber client, not primary (#5)
2. **[CRITICAL]** Fix `waitForMessage` `const` re-assignment — causes memory leak (#6)
3. **[CRITICAL]** Fix `acquireLock` overwriting lock key with data (#3)
4. **[CRITICAL]** Fix empty string TTL in optimistic scripts (#4, #18)
5. **[HIGH]** Remove `ioredis` from `dependencies`, keep in `peerDependencies` (#9)
6. **[HIGH]** Call `rollbackWrite` on partial optimistic write failures (#10)
7. **[HIGH]** Fix always-passing stress test assertion (#22)
8. **[HIGH]** Add tests for hybrid strategy, cache, and compatibility API (#26)
9. **[MEDIUM]** Fix `onError` not propagating errors in `Lock.using()` (#13)
10. **[MEDIUM]** Fix double-release in `InternalRedlock.using()` (#14)
11. **[MEDIUM]** Actually integrate CacheManager into acquire/release flow (#12)
12. **[MEDIUM]** Fix PubSub waiter to listen on all locked resources (#16)
